    ...
    <form action={handleSubmit} className="space-y-6">
      ...
      {mode === 'create' && (
        <div className="space-y-2">
          ...
          {fieldErrors.eventDate && (
            <p className="text-sm text-destructive">{fieldErrors.eventDate[0]}</p>
          )}
        </div>
      )}
      ...
      {mode === 'edit' && event && (
        <div className="space-y-2">
          <Label>이벤트 일시</Label>
          <Input
            value={isoToLocalDatetime(event.eventDate)}
            disabled
            className="bg-muted"
          />
          <p className="text-xs text-muted-foreground">이벤트 날짜는 수정할 수 없습니다.</p>
        </div>
      )}
      ...
      {mode === 'create' && (
        <div className="space-y-2">
          ...
          {fieldErrors.maxCapacity && (
            <p className="text-sm text-destructive">{fieldErrors.maxCapacity[0]}</p>
          )}
        </div>
      )}
      ...
      {mode === 'edit' && event && (
        <div className="space-y-2">
          <Label>최대 정원</Label>
          <Input
            value={`${event.maxCapacity}명`}
            disabled
            className="bg-muted"
          />
          <p className="text-xs text-muted-foreground">정원은 수정할 수 없습니다.</p>
        </div>
      )}
