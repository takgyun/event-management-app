 ...
 *
 * 시나리오: 정원 2명 이벤트에 3명 신청 → 2번째 참가자 취소 → 3번째 자동 승격
 *
 * 검증 항목:
...
test.describe('Waitlist 및 자동 승격', () => {
  ...
  test.beforeAll(() => {
    hostEmail = generateTestEmail('waitlist-host');
    userAEmail = generateTestEmail('waitlist-usera');
    userBEmail = generateTestEmail('waitlist-userb');
    userCEmail = generateTestEmail('waitlist-userc');
  });
  ...
  test('정원 초과 시 waitlist 처리 및 자동 승격', async ({ page }) => {
    ...
    // ================================================================
    await test.step('호스트 회원가입 및 정원 2명 이벤트 생성', async () => {
    ...
    // ================================================================
    // 2단계: 호스트 로그아웃 후 사용자 A 회원가입 및 참가 신청
    // ================================================================
    ...
    // ================================================================
    // 3단계: 사용자 A 로그아웃 후 사용자 B 회원가입 및 참가 신청
    // ================================================================
    ...
    // ================================================================
    // 4단계: 사용자 B 로그아웃 후 사용자 C 회원가입 및 참가 신청 → waitlist
    // ================================================================
    await test.step('사용자 C 참가 신청 → waitlist (정원 초과)', async () => {
      await signOut(page);
      await signUp(page, userCEmail, testPassword);

      if (!page.url().includes('dashboard')) {
        await signIn(page, userCEmail, testPassword);
      }
